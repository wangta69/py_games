    ...
    for event in pygame.event.get():
        if event.type == QUIT:
           ...
           sys.exit()
            ...
            print('MOUSEBUTTONDOWN', event.button, pygame.mouse.get_pos(), event.pos)
        elif event.type == MOUSEBUTTONUP:  # 마우스 버튼을 업할때 발생
            print('MOUSEBUTTONUP', event.button, pygame.mouse.get_pos(), event.pos)
        elif event.type == MOUSEMOTION:  # 마우스가 움질일때 발생
            print('MOUSEMOTION', pygame.mouse.get_pos(), event.pos)
